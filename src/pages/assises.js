...
class Assises extends React.Component {
  render(){
    //print out all data under a class Assises
    let data = this.props.data.allAirtable.nodes.map(node => (
    ...
    //print out 
    let assises = [];
    this.props.data.allAirtable.nodes.map((node, index) => (
      node.data.Sub_Categories.map(subcategory => (
      assises.push(subcategory)  
      ))
    ))
    console.log(assises)
    // Items displayed
